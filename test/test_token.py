...
class TestToken:
    ...
    def test_simple_token(self):
        ...
        assert token.get_expires_at_ms() == pytest.approx((datetime.now(timezone.utc).timestamp() * 1000) + 100, 10)
        assert token.get_received_at_ms() == pytest.approx((datetime.now(timezone.utc).timestamp() * 1000), 10)
    ...
    def test_jwt_token(self):
        ...
        assert jwt_token.get_received_at_ms() == pytest.approx((datetime.now(timezone.utc).timestamp() * 1000), 10)
